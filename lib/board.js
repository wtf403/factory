...
export async function createBoard({ owner, title = 'Agent Factory' }) {
  ...
  const mk = q(['-f', 'query=mutation($ownerId:ID!,$title:String!){createProjectV2(input:{ownerId:$ownerId,title:$title}){projectV2{id number url}}}',
    '-f', `ownerId=${ownerId}`, '-f', `title=${title}`]);
  const proj = mk.data.createProjectV2.projectV2;

  const mkfield = (name, type) => q([
    '-f', 'query=mutation($pid:ID!,$name:String!,$type:ProjectV2FieldType!){createProjectV2Field(input:{projectId:$pid,name:$name,dataType:$type}){projectV2Field{... on ProjectV2Field{id} ... on ProjectV2SingleSelectField{id}}}}',
    '-f', `pid=${proj.id}`, '-f', `name=${name}`, '-f', `type=${type}`,
  ]).data.createProjectV2Field.projectV2Field.id;

  const setopts = (fid, opts) => q([
    '-f', 'query=mutation($fid:ID!,$opts:[ProjectV2SingleSelectFieldOptionInput!]!){updateProjectV2Field(input:{fieldId:$fid,singleSelectOptions:$opts}){projectV2Field{... on ProjectV2SingleSelectField{id}}}}',
