...
export async function stepApp({ appSlug, dry, skip }) {
  s.log.step(`GitHub App: ${appSlug}`);
  if (dry || skip) { note('App step skipped.'); return { opened: false }; }
  const { installUrl } = await import('./app.js');
  const url = installUrl(appSlug);
  note(`Install page: ${url}`);
  // Never block on the opener: headless/CI environments may never resolve it.
  await Promise.race([openInstallPage(appSlug).catch(() => {}), new Promise((r) => setTimeout(r, 8000))]);
